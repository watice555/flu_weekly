...
**一键查询（macOS）：** 在 Finder 中双击项目根目录的 [查询五类来源.command](查询五类来源.command)，输入 `24/7` 后回车，即可查看南北方的五类来源值和各列对应的报告期次。直接回车默认查询 `24/7`，可连续查询其他周，输入 `q` 退出。缺值显示 `—`，不会填成 0。

也可以在终端直接指定目标周，无需进入项目目录：

```bash
~/Projects_local/flu_weekly/查询五类来源.command 24/7
```

支持 `24/7`、`2024/7`、`2024-W07`。脚本只读本地数据库，不联网、不下载、不重建，也不修改网站数据；需要已经建立项目 `.venv` 和数据库。
