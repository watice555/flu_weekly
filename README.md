...
增量构建根据 PDF 的 SHA-256 和提取器版本跳过已提取文件；同一期的不同 PDF 内容保留为不同版本。构建同时导入当周值与后续报告中的历史引用，已有数据库会自动补入引用，无需删除或重建原值。任一文件无法确认周次、日期、南北方数值或完整引用段落时，整批导入回滚，已生成网页数据保持不变。
...
网站绘图使用每期报告正文的**当周发布值**，不从图像估读、不插值，也不直接用后续报告的“往年同期”或“前一周”引用覆盖。部分后续引用与当期数值不同，可能涉及修订或历史周次对齐；这里不猜测原因。页面的周变化也按相邻两期的当周发布值计算，因此可能与最新 PDF 中的周变化表述不同。
...
| `report_references` | 各 PDF 版本引用的目标年/周、地区、数值、页码、原文及比较措辞 |
| `reference_extractions` | 引用提取器版本、导入时间、重复引用冲突与年份/措辞异常 |
| `observation_sources` | 按目标周与地区逐条列出当期及后续来源，保留来源报告和证据 |
| `weekly_source_values` | 每个目标周、每个地区一行，展开为五类来源值；缺值为 SQL `NULL` |
...
这里的 `24/7` 指 **2024 年第 7 周**，不是 2024 年 7 月。以它为目标周：

**一键查看全部周次（macOS）：** 在 Finder 中双击项目根目录的 [查询五类来源.command](查询五类来源.command)，自动生成并在默认浏览器打开完整表格，**无需逐周输入**。每个目标周分南方、北方两行，五列并列显示各来源值，缺值留空；可以按目标年份、地区筛选，或搜索 `24/7`、`2024-W07`。数值链接到官方 PDF，悬停可查看来源报告期次及页码。

打开时默认选择数据库中最新的目标年份及南方（目前为 2026 年、南方）；“清除筛选”仍显示全部年份和地区。其他来源以同一目标周、同一地区的当期报告值为基准：**完全相同标绿色，不同标红色**，按报告的一位小数精度比较，不另设容差。当期基准值保持中性色；没有当期值时，其他来源显示灰色，不做一致性判断；缺失的来源值仍留空。颜色不表示高低方向，悬停提示和无障碍标签也包含比较结果。

表格保存在被 Git 忽略的 `data/source_table/index.html`，可直接打开。每次双击脚本都会根据当前数据库重新生成，不需要本地服务器，也不会上传网页。仅生成表格、不自动打开浏览器：

```bash
~/Projects_local/flu_weekly/查询五类来源.command --no-open
```

脚本只读本地数据库，不联网、不下载、不重建数据库，也不修改公开网站数据；需要已经建立项目 `.venv` 和数据库。生成页包含全部已收录的目标周（包括没有当期原报告的早年引用），不把缺失周插值补齐。原有单周终端查询仍可通过 `.venv/bin/python -m flu_data.query_sources 24/7` 使用。

| 查询列 | 来源报告 | 提取内容 |
| --- | --- | --- |
| `current_report` | 2024-W07 | 当期报告中的当周值 |
| `next_week` | 2024-W08 | 下一周报告中的“前一周”值 |
| `year_plus_1` | 2025-W07 | 后一年报告引用的 2024 年同期值 |
| `year_plus_2` | 2026-W07 | 后两年报告引用的 2024 年同期值 |
| `year_plus_3` | 2027-W07 | 后三年报告引用的 2024 年同期值，尚未取得时留空 |

查看五列值（单位均为 %）：

```sql
SELECT target_report_id, region,
       current_report, next_week, year_plus_1, year_plus_2, year_plus_3
FROM weekly_source_values
WHERE target_report_id = '2024-W07'
ORDER BY region;
```

查看每个数值来自哪份报告、哪一页及哪段原文：

```sql
SELECT target_report_id, region, source_role, source_report_id,
       ili_percent, pages_json, evidence_json, source_sha256, pdf_url
FROM observation_sources
WHERE target_report_id = '2024-W07'
ORDER BY region, source_year, source_week;
```

- 底层按“来源 PDF 版本＋地区＋引用类型＋目标周”分别存储，不按数值去重。即使五个来源完全一致，也会保留五个独立来源；同一报告摘要与正文的一致重复表述合并成一个来源，保留全部证据。
- 没有来源或没有可确认的目标引用时，五列表中保持 `NULL`，不写成 0、不复制其他来源填空。没有当期原报告的早年引用也保存，当期列留空。
- 下一周按实际周历处理年末跨年和第 53 周；历史引用按原文标注的年份与周号对应。异常年份不自动纠正。比如 2025-W01 原文未引用 2024 年，则 2024-W01 的 `year_plus_1` 留空；超出后三年的引用在明细中记为 `outside_window`，保留证据，但不挪入五列。
- 重复段落对同一引用给出冲突值时，该引用不入数值表，冲突候选值和原文存入 `reference_extractions.diagnostics_json`。不能完整解析的报告会阻止本批导入，不会把解析失败伪装成普通缺值。
- 查询视图使用各来源报告当前采用的 PDF 版本。PDF 内容变更时，旧版本的引用仍保留在 `report_references`，通过校验值关联 `report_versions` 查询。
- **网站仍只导出 `current_observations` 中的当期值**。新增引用不会补齐网页的早年缺周，也不会改变曲线和周变化的计算口径。多来源值仅保存在本地数据库，不进入公开 JSON。
...
这些审计文件是运行命令时的快照。日常数据库构建会更新上述多来源表，但不会自动重新生成审计 HTML/Markdown/JSON；需要最新差异统计时重新运行审计命令。
...
它同时记录官方来源，更新数据库中的当期值、历史引用与网页 JSON。有了后续报告，对应五列表中的空位会自动出现新值；网页仍只使用当期值。没有新 PDF 时也会离线重建索引，但不重复下载或复制 PDF。下载中断或收到非 PDF 响应不会覆盖已有文件；数据库构建失败不会推进 `state.json` 的成功状态。
